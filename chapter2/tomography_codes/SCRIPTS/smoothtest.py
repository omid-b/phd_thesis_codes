...
about = "This script carries out the smooth tests for Sergei Lebedev's tomography code\n"

usage = '''
Usage:
  python smoothtest.py  inbac  iniac  inxc  outputdir
'''
# Note: one can input 'inbac_outl' instead of 'inbac' file
...
smoothing_values = [0.00001, 0.00005, 0.0001, 0.0003, 0.0005, 0.0008,\
                  ...
                  2.5000, 3.0000, 4.0000, 5.0000, 7.0000, 10.000,\
                  20.000]

# Notes: 1) all other parameters are fixed and read from the inputs
#           e.g., "norm damping values" and "gradient damping values" 
#        2) when running smooth tests for isotropic component, smoothing
#           parameters for the anisotropic are fixed (read from 'iniac');
#           similar logic is applied when running smooth tests for the 
...
report = f'\n\n Period: {period} s\n #smoothing values: {len(smoothing_values)}\n smoothing values (min, max):  {min(smoothing_values)}  {max(smoothing_values)}\n output directory: {outputdir}\n\n'
...
if os.path.isdir(outputdir):
    rmtree(outputdir)
    os.mkdir(outputdir)
    os.mkdir(os.path.join(outputdir,'smoothtest_isotropic'))
    os.mkdir(os.path.join(outputdir,'smoothtest_anisotropic'))
else:
    os.mkdir(outputdir)
    os.mkdir(os.path.join(outputdir,'smoothtest_isotropic'))
    os.mkdir(os.path.join(outputdir,'smoothtest_anisotropic'))
...
def write_iniac(iniacdir, smth):
    fn = open(os.path.join(iniacdir,'iniac'), 'w')
    iniac_txt = [f'{max_iter}', f'{int_sol}', iters, dmp, f'{smth[0]} {smth[1]} {smth[2]}', grad_dmp, '0 0 0', '0 0 0']
    fn.write('\n'.join(iniac_txt))
    fn.close()
...
for val in smoothing_values:
    # isotropic tests
    invdirs.append(os.path.join(outputdir,'smoothtest_isotropic','%08.5f-%08.5f-%08.5f' %(val, float(smth.split()[1]), float(smth.split()[2]))))
    os.mkdir(invdirs[-1])
    write_inbac(invdirs[-1])
    write_iniac(invdirs[-1], [val, smth.split()[1], smth.split()[2]])
    write_inxc(invdirs[-1])

    # anisotropic tests
    invdirs.append(os.path.join(outputdir,'smoothtest_anisotropic','%08.5f-%08.5f-%08.5f' %(float(smth.split()[0]), val, val)))
    os.mkdir(invdirs[-1])
    write_inbac(invdirs[-1])
    write_iniac(invdirs[-1], [smth.split()[0], val, val])
    write_inxc(invdirs[-1])
