...
about = "This script carries out the damp tests for Sergei Lebedev's tomography code\n"

usage = '''
Usage:
  python damptest.py  inbac  iniac  inxc  outputdir
'''
...
damping_values = [0.00001, 0.00004, 0.00008, 0.0001, 0.0005,\
               0.0010, 0.0050, 0.0100, 0.0330, 0.05000 ,0.07000, 0.1000,\
               0.1330, 0.1500,0.1660, 0.2000, 0.2330, 0.2660, 0.3000,\
               0.3330, 0.3660, 0.4000, 0.4500]

# Notes: 1) all other parameters are fixed and read from the inputs
#           e.g., "smoothing values" and "gradient damping values" 
...
report = f'\n\n Period: {period} s\n #damping values: {len(damping_values)}\n damping values (min, max):  {min(damping_values)}  {max(damping_values)}\n output directory: {outputdir}\n\n'
...
if os.path.isdir(outputdir):
    rmtree(outputdir)
    os.mkdir(outputdir)
    os.mkdir(os.path.join(outputdir,'damptest_isotropic'))
    os.mkdir(os.path.join(outputdir,'damptest_anisotropic'))
else:
    os.mkdir(outputdir)
    os.mkdir(os.path.join(outputdir,'damptest_isotropic'))
    os.mkdir(os.path.join(outputdir,'damptest_anisotropic'))
...
def write_iniac(iniacdir, dmp):
    fn = open(os.path.join(iniacdir,'iniac'), 'w')
    iniac_txt = [f'{max_iter}', f'{int_sol}', iters, f'{dmp[0]} {dmp[1]} {dmp[2]}', smth, grad_dmp, '0 0 0', '0 0 0']
    fn.write('\n'.join(iniac_txt))
    fn.close()
...
for val in damping_values:
    # isotropic tests
    invdirs.append(os.path.join(outputdir,'damptest_isotropic','%08.5f-%08.5f-%08.5f' %(val, float(dmp.split()[1]), float(dmp.split()[2]))))
    os.mkdir(invdirs[-1])
    write_inbac(invdirs[-1])
    write_iniac(invdirs[-1], [val, dmp.split()[1], dmp.split()[2]])
    write_inxc(invdirs[-1])

    # anisotropic tests
    invdirs.append(os.path.join(outputdir,'damptest_anisotropic','%08.5f-%08.5f-%08.5f' %(float(dmp.split()[0]), val, val)))
    os.mkdir(invdirs[-1])
    write_inbac(invdirs[-1])
    write_iniac(invdirs[-1], [dmp.split()[0], val, val])
    write_inxc(invdirs[-1])
