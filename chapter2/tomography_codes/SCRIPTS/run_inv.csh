#!/bin/csh
# This script automates running "bac/iac/xsc" codes (before outlier exclusion process)
# Usage: csh run_inv.csh runsetdir
...
if (! -e $runsetdir/datalist) then
    ...
    exit
else if (! -e $runsetdir/inbac) then
    printf "\nError!\n Could not find 'inbac' in the given runset!\n\n"
    exit
endif
...
$softwaredir/BIN/bac < $runsetdir/inbac > outbac
...
foreach prd ($prds)
    ...
    cp $runsetdir/iniac $runsetdir/inv/$prd/iniac
    cp $runsetdir/inxc $runsetdir/inv/$prd/inxc
end

rm colsums* ers* matrix* path* rhs* shell* stations* tri*
cp $runsetdir/datalist $runsetdir/inv/
cp $runsetdir/inbac $runsetdir/inv/
cp $runsetdir/iniac $runsetdir/inv/
cp $runsetdir/inxc $runsetdir/inv/
