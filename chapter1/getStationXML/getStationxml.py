#!/usr/bin/env python3
about="This script automates downloading a list of station xml data from IRIS using the FetchData perl script.\n"
usage='''
USAGE:
...
longitude_range = [-180, 180] #a list in [minLon, maxLon] format (limiting station location)
latitude_range  = [-90, 90]   #a list in [minLat, maxLat] format (limiting station location)
FetchData_Script = './FetchData-2018.337' #path to IRIS 'FetchData' perl script
#Note: download the latest IRIS 'FetchData' perl script from http://service.iris.edu/clients/
#==============================#
...
def getxml(sta, net, chn, outfile):
    ...
    shell_cmd= f"perl {FetchData_Script} -S {sta} -N {net} -C {chn} -s {start_time} -e {end_time} -X {outfile} -q\n"
...
while i < len(station):
    ierr=0
    print(f"  Downloding xml data for {network[i]}.{station[i]}",end="    \r")
    for chn in channel:
