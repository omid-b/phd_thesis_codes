#!/usr/bin/env python3
about="This script performs instrument response removal using station info xml files.\n"
usage = '''
USAGE:
  python3 removeResposne.py  <stationxml list>  <input datasets dir> <output datasets dir>

*Note: This script also reads stla and stlo from xml data and writes the sac headers (using SAC) into the output.
'''
#UPDATE: 17 Jan 2020
...
pre_filt = (0.005, 0.006, 30.0, 35.0) #applied in st.remove_response() process; see obspy documentation for more info
SAC= "/usr/local/sac/bin/sac" #path to SAC software; only used to write sac headers
#===============================#
...
if input_datasets == output_datasets:
    print(f"{usage}\n\nError!\n\n output and input datasets directories should not be the same!\n")
    exit() 
...
if len(event) == 0:
    print(f"{usage}\n\nError! No event directory was found! Check 'event_regex' parameter\n event_regex={event_regex}\n\n")
    exit()
else:
    event.sort()
...
for evt in event:
    ...
    for x in saclist:
        if re.search(sacfile_regex, x):
            ...
            sacfileTag.append(st[0].stats.network+'.'+st[0].stats.station+'.'+st[0].stats.channel)
...
#====Functions=====#
def write_sac_headers(stla, stlo, stel, sacfile):
...
while i < len(sacfile_src):
    ...
    try:
        ...
        write_sac_headers(stla, stlo, stel, sacfile_dst[i])
    except ValueError:
        error.append(sacfile_src[i])
    progress= "%.0f" %(((i+1) / len(sacfile_src))*100)
    print(f"  Program progress:  {progress}% (#Errors: {len(error)})", end="    \r")
    i+=1
...
if len(error)>0:
    errorlog=open(os.path.join(output_datasets,'removeResponse_errors.log'),'w')
    for x in error:
        errorlog.write(f"{x}\n")
    errorlog.close()
    print(f"Program failed to remove instrument response in {len(error)} cases!\nCheck 'removeResponse_errors.log'\n")
