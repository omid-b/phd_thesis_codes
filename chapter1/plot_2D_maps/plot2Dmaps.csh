#!/bin/csh
#This script uses GMT to plot maps of any xyz scattered data.
# USAGE: ./plot2Dmaps.csh <datalist> <mask polygon (optional)>
# Coded By: omid.bagherpur@gmail.com
# UPDATE: April 4, 2019
#========Adjustable Parameters==========#
set uniform_scale = 'no' # 'yes' or 'no'; uniform colorscale for all maps
set min_cpt = 'auto' #auto or a number
set max_cpt = 'auto' #auto or a number
set plot_perturbation   = 'no' # 'yes' or 'no'; if 'yes', <datalist> should have two columns: 1)data, 2)average value
set plot_contours = 'no' #'yes' or 'no'
set plot_nodes    = 'no' #'yes' or 'no'
set scale_label = 'Vs (km/s)'
set scale_annot = 'auto' #choose a value or 'auto'
set map_titles = 'no' #If 'yes' and <map title> is provided, the title will be placed at the top of maps

#GMT parameters:
set plot_style = 'poster'  # 'paper' or 'poster' 'talk'
...
set box_map  = 'no' # 'yes' or 'no'; rectangular map 
...
set margin_bottom = 1.5 #grid bottom margin (in degrees) 0.5
#omid: my region proper margins for box map: 1.5, 0.5, 0.25, 0.5
#=======================================#
clear
printf "This script uses GMT to plot maps of any xyz scattered data.\n\n"
#Plot styles
if ($plot_style == 'paper') then
  set font1 = '22p,Helvetica,black' #FONT_ANNOT_PRIMARY
  set font2 = '18p,Helvetica,black' #FONT_ANNOT_SECONDARY
  set font3 = '22p,Helvetica,black' #FONT_LABEL
  set font4 = '32p,Helvetica-Bold,black' #FONT_TITLE
  set frame = 'MAP_FRAME_TYPE plain MAP_FRAME_PEN thin,black MAP_FRAME_WIDTH 0.1c'
  set annot = '-Bx3 -By2.5 -BWsNe'    #pscoast -B flags
  set scale_flag = '-Dx0p/-50p+w800p/30p+jTL+h'
else if ($plot_style == 'poster') then
  set font1 = '24p,Helvetica,black' #FONT_ANNOT_PRIMARY 36p
...
else if ($plot_style == 'talk') then
  set font1 = '22p,Helvetica,black' #FONT_ANNOT_PRIMARY
  set font2 = '22p,Helvetica,black' #FONT_ANNOT_SECONDARY
  set font3 = '22p,Helvetica,black' #FONT_LABEL
  set font4 = '30p,Helvetica,black' #FONT_TITLE
  set frame = 'thinnest,black'      #MAP_FRAME_PEN
  set annot = "-Bx4 -By3 -BWsNe"    #pscoast -B flags
  set scale_flag = '-Dx0p/-50p+w780p/30p+jTL+h'
else
  printf "Error plot style! check 'Adjustable Parameters'.\n\n"
  exit
endif
...
#check input
if ($#argv == 0 ) then
  printf "Error!\n USAGE: $0 <datalist> <mask polygon> <map titles>\n Note: <mask polygon> and <map titles> are optional\n\n"
  exit
else
  set datalist = $argv[1]
...
if ($plot_perturbation == 'yes' && $ncol_datalist == 1) then
  printf "WARNING! No average values for datasets are provided in <datalist>\n > Only maps of absolute values will be generated!\n\n"
  set plot_perturbation = 'no'
endif
...
while ($i < $nData)
  ...
  gmt surface bm.tmp -I$grdSpacing -R$reg -T$tension_factor -G$fn.tmp0
  
  if ($plot_perturbation == 'yes') then
    set avg = `cat datalist.tmp|awk '{print $2}'|awk "NR==$i"`
    gmt grdmath $fn.tmp0 $avg SUB $fn.tmp0 DIV 100 MUL = $fn.tmp
  else
    mv $fn.tmp0 $fn.tmp
  endif
...
printf "  Making '*.nc' files  ... Done.\n"

#make the colorscale symetric around the average:
if ($plot_perturbation == 'yes') then
  set cpt_l1ABS = `echo $cpt_l1|awk '{printf "%f\n",(-1)*$1}'`

  if (`echo "$cpt_l1ABS > $cpt_l2"|bc` == 1) then
    set cpt_l2 = $cpt_l1ABS
  else
    set cpt_l1 = `echo $cpt_l2|awk '{printf "%f\n",(-1)*$1}'`
  endif
endif

#make plots

printf "  Generating plots ... \r"
@ i=0
foreach grd (`ls *.nc|grep -v mask.nc`)
  ...
  if ($#argv == 3) then
    ...
    if ($plot_style == 'paper' &&  $map_titles == 'yes') then
      set annot2 = `echo "$annot"|awk -v tt="$title_str" '{printf "%s+t%s",$0,tt}'|sed 's/,,/\\n/g'`
    endif
    if ($plot_style == 'poster' && $map_titles == 'yes') then
      set annot2 = `echo "$annot"|awk -v tt="$title_str" '{printf "%s+t%s",$0,tt}'|sed 's/,,/\\n/g'`
    endif
    if ($plot_style == 'talk' && $map_titles == 'yes') then
      set annot2 = `echo "$annot"|awk -v tt="$title_str" '{printf "%s+t%s",$0,tt}'|sed 's/,,/\\n/g'`
    endif
    if ($map_titles != 'yes') then
