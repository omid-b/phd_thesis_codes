#!/usr/bin/env python3
about = "This script calculates RMS values for acausal, causal, \
and symmetrized component of the original EGFs inside a directory."
usage = "Usage: python3 calc_rms.py <EGF_dir> <out_file>"
...
def calc_rms(signal_or_noise):
    rms = np.sqrt(np.nanmean(np.square(signal_or_noise)))
    return rms
...
for i in range(len(egf_list)):
    ...
    causal_rms_noise.append(calc_rms(causal_noise_data))
    causal_rms_signal.append(calc_rms(causal_signal_data))
    ...
    acausal_rms_noise.append(calc_rms(acausal_noise_data))
    acausal_rms_signal.append(calc_rms(acausal_signal_data))
...
for i in range(len(egf_list)):
    ...
    # only output if snr values are positive
    if sym_snr[i] > 0:
	    fopen.write("%4s %4s %3.0f %3.0f %5.0f %5d %.4f %.4f %5.1f\n"\
              %(sta_pairs[i][0], sta_pairs[i][1], float(headers['az']),\
              float(headers['baz']), float(headers['dist']),\
              int(headers['kevnm']), float(acausal_rms_noise[i]),\
              float(causal_rms_noise[i]), float(sym_snr[i])))
